...
function CheckoutSuccessContent() {
  ...
  useEffect(() => {
    // Fallback order ID in case query param is missing
    setOrderId(Math.floor(Math.random() * 100000).toString());
  }, []);

  const orderNumber = searchParams.get("order_number") || `GN-${orderId}`;
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12">
      <Suspense fallback={
        <div className="text-center">
          <CheckCircle2 className="h-12 w-12 text-success animate-pulse mx-auto mb-4" />
